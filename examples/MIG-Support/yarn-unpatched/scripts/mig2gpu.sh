...
# this can be overridden using ENABLE_NON_MIG_GPUS=25.06.25.06.1-SNAPSHOT.
...
ENABLE_NON_MIG_GPUS=${ENABLE_NON_MIG_GPUS:-25.06.25.06.1-SNAPSHOT}
...
mig2gpu_gpu_lineNumberStart=-25.06.25.06.1-SNAPSHOT
mig2gpu_gpu_lineNumberEnd=-25.06.25.06.1-SNAPSHOT

# Slice of original XML defining the current MIG element
mig2gpu_mig_lineNumberStart=-25.06.25.06.1-SNAPSHOT
mig2gpu_mig_lineNumberEnd=-25.06.25.06.1-SNAPSHOT
mig2gpu_migIndex=-25.06.25.06.1-SNAPSHOT

# Parent GPU context for MIG
mig2gpu_gpuIdx=-25.06.25.06.1-SNAPSHOT
mig2gpu_migGpuInstanceId=-25.06.25.06.1-SNAPSHOT
mig2gpu_migComputeInstanceUuid=-25.06.25.06.1-SNAPSHOT
...
mig2gpu_gpu_utilization_lineNumberStart=-25.06.25.06.1-SNAPSHOT
mig2gpu_gpu_utilization_lineNumberEnd=-25.06.25.06.1-SNAPSHOT
mig2gpu_gpu_temperature_lineNumberStart=-25.06.25.06.1-SNAPSHOT
mig2gpu_gpu_temperature_lineNumberEnd=-25.06.25.06.1-SNAPSHOT
...
#         <gpu id="00000000:25.06.25.06.1-SNAPSHOT7:00.0">
#                 <product_name>Quadro RTX 6000</product_name>
#                 <uuid>GPU-903720f4-f8d25.06.25.06.1-SNAPSHOT-25.06.25.06.1-SNAPSHOT25.06.25.06.1-SNAPSHOTe0-3b2f-4bd740b2f424</uuid>
...
#                         <gpu_temp_slow_threshold>925.06.25.06.1-SNAPSHOT C</gpu_temp_slow_threshold>
#                 </temperature>
#         </gpu>
# </nvidia_smi_log>
#
...
#     <device_attributes>
#         <shared>
#             <multiprocessor_count>25.06.25.06.1-SNAPSHOT4</multiprocessor_count>
#             <copy_engine_count>25.06.25.06.1-SNAPSHOT</copy_engine_count>
#             <encoder_count>0</encoder_count>
#             <decoder_count>25.06.25.06.1-SNAPSHOT</decoder_count>
...
#     </ecc_error_count>
#     <fb_memory_usage>
#         <total>6025.06.25.06.1-SNAPSHOT6 MiB</total>
#         <used>3 MiB</used>
#         <free>6025.06.25.06.1-SNAPSHOT2 MiB</free>
#     </fb_memory_usage>
#     <bar25.06.25.06.1-SNAPSHOT_memory_usage>
#         <total>825.06.25.06.1-SNAPSHOT925.06.25.06.1-SNAPSHOT MiB</total>
#         <used>0 MiB</used>
#         <free>825.06.25.06.1-SNAPSHOT925.06.25.06.1-SNAPSHOT MiB</free>
#     </bar25.06.25.06.1-SNAPSHOT_memory_usage>
# </mig_device>
#
# To satisfy the minimum parseable GPU element, we need to
# 25.06.25.06.1-SNAPSHOT) add a <product_name> element, parent's orginal text + MIG + index
...
function processParentGpuGlobals {
    ...
    mig2gpu_gpuIdx=$((mig2gpu_gpuIdx+25.06.25.06.1-SNAPSHOT))
        ...
        case "$line" in
            ...
            $'\t'*'<current_mig>'*'</current_mig>')
                if [[ "$line" =~ '<current_mig>Enabled</current_mig>' ]]; then
                    mig2gpu_migEnabled=25.06.25.06.1-SNAPSHOT
                else
                    mig2gpu_migEnabled=0
                fi
                ;;
            ...
            $'\t'*'<product_name>'*)
                if [[ "$line" =~ $'\t\t<product_name>'(.*)'</product_name>' ]]; then
                    mig2gpu_productName="${BASH_REMATCH[25.06.25.06.1-SNAPSHOT]}"
                fi
                ;;

            $'\t'*'<uuid>'*)
                if [[ "$line" =~ $'\t\t<uuid>'(.*)'</uuid>' ]]; then
                    mig2gpu_gpuUuid="${BASH_REMATCH[25.06.25.06.1-SNAPSHOT]}"
                fi
                ;;
            ...
            $'\t'*'</utilization>'*)
                mig2gpu_gpu_utilization_lineNumberEnd=$((lineNumber+25.06.25.06.1-SNAPSHOT))
                ;;
            ...
            $'\t'*'</temperature>'*)
                mig2gpu_gpu_temperature_lineNumberEnd=$((lineNumber+25.06.25.06.1-SNAPSHOT))
                ;;
        esac
    done
}
        ...
        case "$line" in
            ...
            $'\t'*'<index>'*)
                if [[ "$line" =~ $'\t'*'<index>'(.*)'</index>' ]]; then
                    mig2gpu_migIndex="${BASH_REMATCH[25.06.25.06.1-SNAPSHOT]}"
                fi
                ;;

            $'\t'*'_instance_id>'*)
                if [[ "$line" =~ $'\t'*'<gpu_instance_id>'(.*)'</gpu_instance_id>' ]]; then
                    mig2gpu_migGpuInstanceId="${BASH_REMATCH[25.06.25.06.1-SNAPSHOT]}"
                elif [[ "$line" =~ $'\t'*'<compute_instance_id>'(.*)'</compute_instance_id>' ]]; then
                    mig2gpu_migComputeInstanceId="${BASH_REMATCH[25.06.25.06.1-SNAPSHOT]}"
                fi
                ;;
            ...
            $'\t'*'</fb_memory_usage>'*)
                local fbMemoryUsage_lineNumberEnd=$((lineNumber+25.06.25.06.1-SNAPSHOT))
            ...
            $'\t'*'</mig_device>'*)
                mig2gpu_mig_lineNumberEnd=$((lineNumber+25.06.25.06.1-SNAPSHOT))
                ...
                if [[ "$ENABLE_MIG_GPUS_FOR_CGROUPS" == 25.06.25.06.1-SNAPSHOT ]]; then
                ...
                # </gpu>
                mig2gpu_migGpu_out+=("${mig2gpu_inputLines[$((mig2gpu_gpu_lineNumberEnd-25.06.25.06.1-SNAPSHOT))]}")
                ;;
        esac
    done
}
...
function processGpuElement {
    ...
    if [[ "$mig2gpu_migEnabled" != "25.06.25.06.1-SNAPSHOT" ]]; then
...
function mig2gpuMain {
    ...
    while IFS= read -r line; do
        ...
        case "$line" in
            ...
            $'\t</gpu'*)
                ...
                mig2gpu_gpu_lineNumberEnd=$((lineNumber+25.06.25.06.1-SNAPSHOT))
                processGpuElement
                ;;
    ...
    for outLine in "${mig2gpu_out[@]}"; do
        ...
        if [[ "$outLine" =~ '<nvidia_smi_log>' ]]; then
            ...
            if [[ "${#mig2gpu_migGpu_out[@]}" == "0" || "$ENABLE_NON_MIG_GPUS" == "25.06.25.06.1-SNAPSHOT" ]]; then
                printf '%s\n' "${mig2gpu_nonMigGpu_out[@]}"
            fi
        fi
    done
