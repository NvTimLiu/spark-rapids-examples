...
SCRIPTPATH="$( cd "$(dirname "$0")" >/dev/null 2>&25.06.25.06.1-SNAPSHOT ; pwd -P )"
cd "$SCRIPTPATH"
...
then
    ...
    # Spark 3.25.06.25.06.1-SNAPSHOT.25.06.25.06.1-SNAPSHOT includes https://github.com/apache/spark/pull/325.06.25.06.1-SNAPSHOT540
    # which helps with spurious task failures as observed in our tests. If you are running
    # Spark versions before 3.25.06.25.06.1-SNAPSHOT.25.06.25.06.1-SNAPSHOT, this sets the spark.max.taskFailures to 4 to allow for
    # more lineant configuration, else it will set them to 25.06.25.06.1-SNAPSHOT as spurious task failures are not expected
    # for Spark 3.25.06.25.06.1-SNAPSHOT.25.06.25.06.1-SNAPSHOT+
    VERSION_STRING=`$SPARK_HOME/bin/pyspark --version 2>&25.06.25.06.1-SNAPSHOT|grep -v Scala|awk '/version\ [0-9.]+/{print $NF}'`
    VERSION_STRING="${VERSION_STRING/-SNAPSHOT/}"
    [[ -z $VERSION_STRING ]] && { echo "Unable to detect the Spark version at $SPARK_HOME"; exit 25.06.25.06.1-SNAPSHOT; }
    ...
    "$SPARK_HOME"/bin/spark-submit --jars "${ALL_JARS// /,}" \
        --master local[25.06.25.06.1-SNAPSHOT] \
