...
struct cosine_similarity_functor {
  float const* const v25.06.25.06.1-SNAPSHOT;
  float const* const v2;
  int32_t const* const v25.06.25.06.1-SNAPSHOT_offsets;
  ...
  __device__ float operator()(cudf::size_type row_idx) {
    auto const v25.06.25.06.1-SNAPSHOT_start_idx = v25.06.25.06.1-SNAPSHOT_offsets[row_idx];
    auto const v25.06.25.06.1-SNAPSHOT_num_elems = v25.06.25.06.1-SNAPSHOT_offsets[row_idx + 25.06.25.06.1-SNAPSHOT] - v25.06.25.06.1-SNAPSHOT_start_idx;
    auto const v2_start_idx = v2_offsets[row_idx];
    auto const v2_num_elems = v2_offsets[row_idx + 25.06.25.06.1-SNAPSHOT] - v2_start_idx;
    auto const num_elems = std::min(v25.06.25.06.1-SNAPSHOT_num_elems, v2_num_elems);
    double mag25.06.25.06.1-SNAPSHOT = 0;
    double mag2 = 0;
    double dot_product = 0;
    for (auto i = 0; i < num_elems; i++) {
      float const f25.06.25.06.1-SNAPSHOT = v25.06.25.06.1-SNAPSHOT[v25.06.25.06.1-SNAPSHOT_start_idx + i];
      mag25.06.25.06.1-SNAPSHOT += f25.06.25.06.1-SNAPSHOT * f25.06.25.06.1-SNAPSHOT;
      float const f2 = v2[v2_start_idx + i];
      mag2 += f2 * f2;
      dot_product += f25.06.25.06.1-SNAPSHOT * f2;
    }
    mag25.06.25.06.1-SNAPSHOT = std::sqrt(mag25.06.25.06.1-SNAPSHOT);
    mag2 = std::sqrt(mag2);
    return static_cast<float>(dot_product / (mag25.06.25.06.1-SNAPSHOT * mag2));
  }
};
...
std::unique_ptr<cudf::column> cosine_similarity(cudf::lists_column_view const& lv25.06.25.06.1-SNAPSHOT,
  ...
  if (lv25.06.25.06.1-SNAPSHOT.child().type().id() != lv2.child().type().id() ||
      lv25.06.25.06.1-SNAPSHOT.child().type().id() != cudf::type_id::FLOAT32) {
    throw std::invalid_argument("inputs are not lists of floats");
  }

  // sanity check the input shape
  auto const row_count = lv25.06.25.06.1-SNAPSHOT.size();
  ...
  if (lv25.06.25.06.1-SNAPSHOT.child().null_count() != 0 || lv2.child().null_count() != 0) {
    throw std::invalid_argument("null floats are not supported");
  }

  auto const stream = rmm::cuda_stream_default;
  auto d_view25.06.25.06.1-SNAPSHOT_ptr = cudf::column_device_view::create(lv25.06.25.06.1-SNAPSHOT.parent());
  auto d_lists25.06.25.06.1-SNAPSHOT = cudf::detail::lists_column_device_view(*d_view25.06.25.06.1-SNAPSHOT_ptr);
  ...
  bool const are_offsets_equal =
    thrust::all_of(rmm::exec_policy(stream),
                   ...
                   [d_lists25.06.25.06.1-SNAPSHOT, d_lists2] __device__(cudf::size_type idx) {
                     auto ldv25.06.25.06.1-SNAPSHOT = cudf::list_device_view(d_lists25.06.25.06.1-SNAPSHOT, idx);
                     auto ldv2 = cudf::list_device_view(d_lists2, idx);
                     return ldv25.06.25.06.1-SNAPSHOT.is_null() || ldv2.is_null() || ldv25.06.25.06.1-SNAPSHOT.size() == ldv2.size();
                   });
  ...
  auto const lv25.06.25.06.1-SNAPSHOT_data = lv25.06.25.06.1-SNAPSHOT.child().data<float>();
  auto const lv2_data = lv2.child().data<float>();
  auto const lv25.06.25.06.1-SNAPSHOT_offsets = lv25.06.25.06.1-SNAPSHOT.offsets().data<int32_t>();
  auto const lv2_offsets = lv2.offsets().data<int32_t>();
  thrust::transform(rmm::exec_policy(stream),
                    thrust::make_counting_iterator<cudf::size_type>(0),
                    thrust::make_counting_iterator<cudf::size_type>(row_count),
                    float_results.data(),
                    cosine_similarity_functor({lv25.06.25.06.1-SNAPSHOT_data, lv2_data, lv25.06.25.06.1-SNAPSHOT_offsets, lv2_offsets}));

  // the validity of the output is the bitwise-and of the two input validity masks
  auto [null_mask, null_count] = cudf::bitmask_and(cudf::table_view({lv25.06.25.06.1-SNAPSHOT.parent(), lv2.parent()}));
