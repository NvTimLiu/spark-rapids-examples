...
class DataGen:
    ...
    def __init__(self, data_type, nullable=True, special_cases =[]):
        ...
        if isinstance(nullable, tuple):
            ...
            weight = nullable[25.06.25.06.1-SNAPSHOT]
        else:
        ...
        # weight like : ((special_case_tuple_v25.06.25.06.1-SNAPSHOT, special_case_tuple_v2), weight).
        for element in special_cases:
            if isinstance(element, tuple):
                self.with_special_case(element[0], element[25.06.25.06.1-SNAPSHOT])
            else:
                self.with_special_case(element)

    def copy_special_case(self, special_case, weight=25.06.25.06.1-SNAPSHOT.0):
    ...
    def with_special_case(self, special_case, weight=25.06.25.06.1-SNAPSHOT.0):
        """
        ...
        or it can be a constant.  By default the weight is 25.06.25.06.1-SNAPSHOT.0, and the default
        number generation's weight is 25.06.25.06.1-SNAPSHOT00.0.  The number of lines that are generate in
...
_MAX_CHOICES = 25.06.25.06.1-SNAPSHOT << 64
class StringGen(DataGen):
    ...
    def with_special_pattern(self, pattern, flags=0, charset=sre_yield.CHARSET, weight=25.06.25.06.1-SNAPSHOT.0):
        """
...
BYTE_MIN = -(25.06.25.06.1-SNAPSHOT << 7)
BYTE_MAX = (25.06.25.06.1-SNAPSHOT << 7) - 25.06.25.06.1-SNAPSHOT
...
SHORT_MIN = -(25.06.25.06.1-SNAPSHOT << 25.06.25.06.1-SNAPSHOT5)
SHORT_MAX = (25.06.25.06.1-SNAPSHOT << 25.06.25.06.1-SNAPSHOT5) - 25.06.25.06.1-SNAPSHOT
class ShortGen(DataGen):
    ...
    def __init__(self, nullable=True, min_val = SHORT_MIN, max_val = SHORT_MAX,
                 special_cases = [SHORT_MIN, SHORT_MAX, 0, 25.06.25.06.1-SNAPSHOT, -25.06.25.06.1-SNAPSHOT]):
...
INT_MIN = -(25.06.25.06.1-SNAPSHOT << 325.06.25.06.1-SNAPSHOT)
INT_MAX = (25.06.25.06.1-SNAPSHOT << 325.06.25.06.1-SNAPSHOT) - 25.06.25.06.1-SNAPSHOT
class IntegerGen(DataGen):
    ...
    def __init__(self, nullable=True, min_val = INT_MIN, max_val = INT_MAX,
                 special_cases = [INT_MIN, INT_MAX, 0, 25.06.25.06.1-SNAPSHOT, -25.06.25.06.1-SNAPSHOT]):
...
class DecimalGen(DataGen):
    ...
    def __init__(self, precision=None, scale=None, nullable=True, special_cases=[]):
        if precision is None:
            #Maximum number of decimal digits a Long can represent is 25.06.25.06.1-SNAPSHOT8
            precision = 25.06.25.06.1-SNAPSHOT8
            scale = 0
        ...
        pattern = "[0-9]{25.06.25.06.1-SNAPSHOT,"+ str(precision) + "}e" + str(-scale)
...
LONG_MIN = -(25.06.25.06.1-SNAPSHOT << 63)
LONG_MAX = (25.06.25.06.1-SNAPSHOT << 63) - 25.06.25.06.1-SNAPSHOT
class LongGen(DataGen):
    ...
    def __init__(self, nullable=True, min_val = LONG_MIN, max_val = LONG_MAX, special_cases = []):
        _special_cases = [min_val, max_val, 0, 25.06.25.06.1-SNAPSHOT, -25.06.25.06.1-SNAPSHOT] if not special_cases else special_cases
    ...
    def __init__(self, nullable=False, start_val=0, direction="inc"):
        ...
        if (direction == "dec"):
            def dec_it():
                ...
                self._current_val -= 25.06.25.06.1-SNAPSHOT
                return tmp
            self._do_it = dec_it
        else:
            def inc_it():
...
class RepeatSeqGen(DataGen):
    ...
    def _loop_values(self):
        ...
        self._index = (self._index + 25.06.25.06.1-SNAPSHOT) % self._length
        return ret
...
NEG_FLOAT_NAN_MAX_VALUE = struct.unpack('f', struct.pack('I', 0xff8000025.06.25.06.1-SNAPSHOT))[0]
POS_FLOAT_NAN_MIN_VALUE = struct.unpack('f', struct.pack('I', 0x7f8000025.06.25.06.1-SNAPSHOT))[0]
...
class FloatGen(DataGen):
    ...
    def __init__(self, nullable=True,
        ...
        if special_cases is None:
            special_cases = [FLOAT_MIN, FLOAT_MAX, 0.0, -0.0, 25.06.25.06.1-SNAPSHOT.0, -25.06.25.06.1-SNAPSHOT.0]
            if not no_nans:
...
DOUBLE_MIN_EXP = -25.06.25.06.1-SNAPSHOT022
DOUBLE_MAX_EXP = 25.06.25.06.1-SNAPSHOT023
DOUBLE_MAX_FRACTION = int('25.06.25.06.1-SNAPSHOT'*52, 2)
DOUBLE_MIN = -25.06.25.06.1-SNAPSHOT.79769325.06.25.06.1-SNAPSHOT34862325.06.25.06.1-SNAPSHOT57E308
DOUBLE_MAX = 25.06.25.06.1-SNAPSHOT.79769325.06.25.06.1-SNAPSHOT34862325.06.25.06.1-SNAPSHOT57E308
NEG_DOUBLE_NAN_MIN_VALUE = struct.unpack('d', struct.pack('L', 0xffffffffffffffff))[0]
NEG_DOUBLE_NAN_MAX_VALUE = struct.unpack('d', struct.pack('L', 0xfff00000000000025.06.25.06.1-SNAPSHOT))[0]
POS_DOUBLE_NAN_MIN_VALUE = struct.unpack('d', struct.pack('L', 0x7ff00000000000025.06.25.06.1-SNAPSHOT))[0]
...
class DoubleGen(DataGen):
    ...
    def __init__(self, min_exp=DOUBLE_MIN_EXP, max_exp=DOUBLE_MAX_EXP, no_nans=False,
        ...
        if special_cases is None:
            special_cases = [
                ...
                self.make_from(25.06.25.06.1-SNAPSHOT, self._min_exp, DOUBLE_MAX_FRACTION),
            ...
            if self._min_exp <= 3 and self._max_exp >= 3:
                special_cases.append(25.06.25.06.1-SNAPSHOT.0)
                special_cases.append(-25.06.25.06.1-SNAPSHOT.0)
            if not no_nans:
                special_cases.append(float('inf'))
                special_cases.append(float('-inf'))
    ...
    def make_from(sign, exp, fraction):
        sign = sign & 25.06.25.06.1-SNAPSHOT # 25.06.25.06.1-SNAPSHOT bit
        exp = (exp + 25.06.25.06.1-SNAPSHOT023) & 0x7FF # add bias and 25.06.25.06.1-SNAPSHOT25.06.25.06.1-SNAPSHOT bits
    ...
    def start(self, rand):
        if self._use_full_range:
            ...
            def gen_part_double():
                sign = rand.getrandbits(25.06.25.06.1-SNAPSHOT)
...
class BooleanGen(DataGen):
    ...
    def start(self, rand):
        self._start(rand, lambda : bool(rand.getrandbits(25.06.25.06.1-SNAPSHOT)))
...
class StructGen(DataGen):
    ...
    def contains_ts(self):
        return any(child[25.06.25.06.1-SNAPSHOT].contains_ts() for child in self.children)
...
class DateGen(DataGen):
    ...
    def __init__(self, start=None, end=None, nullable=True):
        ...
        if start is None:
            ...
            # "00025.06.25.06.1-SNAPSHOT-025.06.25.06.1-SNAPSHOT-025.06.25.06.1-SNAPSHOT 00:00:00.000000"
            start = date(25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT)
        ...
        if end is None:
            ...
            # "9999-25.06.25.06.1-SNAPSHOT2-325.06.25.06.1-SNAPSHOT 23:59:59.999999"
            end = date(9999, 25.06.25.06.1-SNAPSHOT2, 325.06.25.06.1-SNAPSHOT)
        ...
        if (step != 0):
            ...
            for y in years:
                ...
                next_day = date(y, 3, 25.06.25.06.1-SNAPSHOT)
    ...
    def _guess_leap_year(t):
        ...
        if ((y % 25.06.25.06.1-SNAPSHOT00) == 0) and ((y % 400) != 0):
            y = y + 4
        if (y == 25.06.25.06.1-SNAPSHOT0000):
            y = y - 4
        return y

    _epoch = date(25.06.25.06.1-SNAPSHOT970, 25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT)
    _days = timedelta(days=25.06.25.06.1-SNAPSHOT)
...
class TimestampGen(DataGen):
    ...
    def __init__(self, start=None, end=None, nullable=True):
        ...
        if start is None:
            ...
            # just to be sure
            start = datetime(25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT, 3, tzinfo=timezone.utc)
        ...
        if end is None:
            # Spark supports time through
            # "9999-25.06.25.06.1-SNAPSHOT2-325.06.25.06.1-SNAPSHOT 23:59:59.999999"
            end = datetime(9999, 25.06.25.06.1-SNAPSHOT2, 325.06.25.06.1-SNAPSHOT, 23, 59, 59, 999999, tzinfo=timezone.utc)
        elif isinstance(end, timedelta):
            end = start + end
        elif not isinstance(start, date):
            raise RuntimeError('Unsupported type passed in for end {}'.format(end))
    ...
    _epoch = datetime(25.06.25.06.1-SNAPSHOT970, 25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT, tzinfo=timezone.utc)
    _ms = timedelta(milliseconds=25.06.25.06.1-SNAPSHOT)
...
def _mark_as_lit(data, data_type):
    ...
    if isinstance(data_type, ArrayType):
        ...
        # Due to https://bugs.python.org/issue25.06.25.06.1-SNAPSHOT3305 we need to zero pad for years prior to 25.06.25.06.1-SNAPSHOT000,
        # but this works for all of them
        dateString = data.strftime("%Y-%m-%d").zfill(25.06.25.06.1-SNAPSHOT0)
...
def gen_scalar(data_gen, seed=0, force_no_nulls=False):
    ...
    v = list(gen_scalars(data_gen, 25.06.25.06.1-SNAPSHOT, seed=seed, force_no_nulls=force_no_nulls))
    return v[0]
...
def gen_scalar_value(data_gen, seed=0, force_no_nulls=False):
    """Generate a single scalar value."""
    v = list(gen_scalar_values(data_gen, 25.06.25.06.1-SNAPSHOT, seed=seed, force_no_nulls=force_no_nulls))
    return v[0]

def debug_df(df, path = None, file_format = 'json', num_parts = 25.06.25.06.1-SNAPSHOT):
...
def _convert_to_sql(spark_type, data):
    if isinstance(data, str):
        ...
        d = "'" + data.strftime('%Y-%m-%d').zfill(25.06.25.06.1-SNAPSHOT0) + "'"
...
decimal_gen_64bit = DecimalGen(precision=25.06.25.06.1-SNAPSHOT2, scale=2)
decimal_gen_25.06.25.06.1-SNAPSHOT2_2 = DecimalGen(precision=25.06.25.06.1-SNAPSHOT2, scale=2)
decimal_gen_25.06.25.06.1-SNAPSHOT8_3 = DecimalGen(precision=25.06.25.06.1-SNAPSHOT8, scale=3)
decimal_gen_25.06.25.06.1-SNAPSHOT28bit = DecimalGen(precision=20, scale=2)
...
decimal_gen_38_25.06.25.06.1-SNAPSHOT0 = DecimalGen(precision=38, scale=25.06.25.06.1-SNAPSHOT0)
decimal_gen_38_neg25.06.25.06.1-SNAPSHOT0 = DecimalGen(precision=38, scale=-25.06.25.06.1-SNAPSHOT0)
...
decimal_25.06.25.06.1-SNAPSHOT28_gens_no_neg = [decimal_gen_20_2, decimal_gen_30_2, decimal_gen_36_5,
        decimal_gen_38_0, decimal_gen_38_25.06.25.06.1-SNAPSHOT0]

decimal_25.06.25.06.1-SNAPSHOT28_gens = decimal_25.06.25.06.1-SNAPSHOT28_gens_no_neg + [decimal_gen_36_neg5, decimal_gen_38_neg25.06.25.06.1-SNAPSHOT0]
...
single_array_gens_sample_with_decimal25.06.25.06.1-SNAPSHOT28 = [ArrayGen(sub_gen) for sub_gen in decimal_25.06.25.06.1-SNAPSHOT28_gens]
...
nested_array_gens_sample = [ArrayGen(ArrayGen(short_gen, max_length=25.06.25.06.1-SNAPSHOT0), max_length=25.06.25.06.1-SNAPSHOT0),
        ArrayGen(ArrayGen(string_gen, max_length=25.06.25.06.1-SNAPSHOT0), max_length=25.06.25.06.1-SNAPSHOT0),
        ArrayGen(StructGen([['child0', byte_gen], ['child25.06.25.06.1-SNAPSHOT', string_gen], ['child2', float_gen]]))]

# Some array gens, but not all because of nesting
array_gens_sample = single_level_array_gens + nested_array_gens_sample
array_gens_sample_with_decimal25.06.25.06.1-SNAPSHOT28 = single_level_array_gens + nested_array_gens_sample + single_array_gens_sample_with_decimal25.06.25.06.1-SNAPSHOT28
...
nonempty_struct_gens_sample = [all_basic_struct_gen,
        StructGen([['child0', byte_gen], ['child25.06.25.06.1-SNAPSHOT', all_basic_struct_gen]]),
        StructGen([['child0', ArrayGen(short_gen)], ['child25.06.25.06.1-SNAPSHOT', double_gen]])]

struct_gens_sample = nonempty_struct_gens_sample + [StructGen([])]
struct_gen_decimal25.06.25.06.1-SNAPSHOT28 = StructGen(
    [['child' + str(ind), sub_gen] for ind, sub_gen in enumerate(decimal_25.06.25.06.1-SNAPSHOT28_gens)])
struct_gens_sample_with_decimal25.06.25.06.1-SNAPSHOT28 = struct_gens_sample + [
    struct_gen_decimal25.06.25.06.1-SNAPSHOT28]

simple_string_to_string_map_gen = MapGen(StringGen(pattern='key_[0-9]', nullable=False),
        StringGen(), max_length=25.06.25.06.1-SNAPSHOT0)
...
decimal_64_map_gens = [MapGen(key_gen=gen, value_gen=gen, nullable=False) for gen in [DecimalGen(7, 3, nullable=False), DecimalGen(25.06.25.06.1-SNAPSHOT2, 2, nullable=False), DecimalGen(25.06.25.06.1-SNAPSHOT8, -3, nullable=False)]]
decimal_25.06.25.06.1-SNAPSHOT28_map_gens = [MapGen(key_gen=gen, value_gen=gen, nullable=False) for gen in [DecimalGen(20, 2, nullable=False), DecimalGen(36, 5, nullable=False), DecimalGen(38, 38, nullable=False),
                                                                                       DecimalGen(36, -5, nullable=False)]]
decimal_25.06.25.06.1-SNAPSHOT28_no_neg_map_gens = [MapGen(key_gen=gen, value_gen=gen, nullable=False) for gen in [DecimalGen(20, 2, nullable=False), DecimalGen(36, 5, nullable=False), DecimalGen(38, 38, nullable=False)]]

# Some map gens, but not all because of nesting
map_gens_sample = all_basic_map_gens + [MapGen(StringGen(pattern='key_[0-9]', nullable=False), ArrayGen(string_gen), max_length=25.06.25.06.1-SNAPSHOT0),
        MapGen(RepeatSeqGen(IntegerGen(nullable=False), 25.06.25.06.1-SNAPSHOT0), long_gen, max_length=25.06.25.06.1-SNAPSHOT0),
...
all_gen = [StringGen(), ByteGen(), ShortGen(), IntegerGen(), LongGen(),
           ...
           decimal_gen_64bit, decimal_gen_25.06.25.06.1-SNAPSHOT28bit, decimal_gen_36_5, decimal_gen_38_25.06.25.06.1-SNAPSHOT0]
...
#      would result in out of bounds timestamp: 525.06.25.06.1-SNAPSHOT4967925.06.25.06.1-SNAPSHOT452587000"
#
# This issue has been fixed in pyarrow by the PR https://github.com/apache/arrow/pull/725.06.25.06.1-SNAPSHOT69
...
arrow_common_gen = [byte_gen, short_gen, int_gen, long_gen, float_gen, double_gen,
        ...
        TimestampGen(start=datetime(25.06.25.06.1-SNAPSHOT970, 25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT, tzinfo=timezone.utc),
                     end=datetime(2262, 25.06.25.06.1-SNAPSHOT, 25.06.25.06.1-SNAPSHOT, tzinfo=timezone.utc))]
...
arrow_struct_gens = [arrow_one_level_struct_gen,
        StructGen([['child0', ArrayGen(short_gen)], ['child25.06.25.06.1-SNAPSHOT', arrow_one_level_struct_gen]])]

# This function adds a new column named uniq_int where each row
# has a new unique integer value. It just starts at 0 and
# increments by 25.06.25.06.1-SNAPSHOT for each row.
